...
instruction = (
        ...
        "You will be given two screenshots from a video game, the first screenshot is a known bug-free reference, "
        "the second screenshot is a test image that may contain glitches or not. Your job is to compare the test image with the first screenshot "
        "and determine whether the test image shows any of the glitch categories listed below. Return exactly this JSON:\n"
        "{\n"
        "    \"reasoning\": \"Brief explanation of how the reference image was used to identify (or rule out) a glitch in the test image.\",\n"
...
def test_single_image(ref_path, test_path):
    base64_image_ref = encode_image(ref_path)
    image_source_ref = {
        "type": "input_image",
        "image_url": f"data:image/jpeg;base64,{base64_image_ref}",
    }
...
def test_from_jsonl(test_jsonl_path, output_file="predictions.jsonl"):
    ...
    with open(test_jsonl_path, "r") as f:
        for idx, line in enumerate(f):
            ...
            obj = json.loads(line)
            ref_path = obj["ref_path"]
            test_path = obj["test_path"]
            true_label = obj['true_label']

            print(f"Testing image {idx + 1}...")

            # Get prediction
            prediction = test_single_image(ref_path, test_path)
...
if __name__ == "__main__":
    ...
    test_from_jsonl(test_jsonl_path, output_file="predictions.jsonl")
